var PageSummaryUtils = {
    ...
    buildSummaryContainer: function (tagCounts) {
        if (!tagCounts || _.isEmpty(tagCounts)) {
            return;
        }
        var summaryContainer = $('<div class="container source jumbotron"></div>');
        var summaryHeader = $('<div class="header tag-summary"><h2>Tag Summary<h2></div>');
        ...
        _.each(
            _.sortBy(_.pairs(tagCounts), 0),
            function(tag) {
                var name = tag[0];
                var count = tag[1];
                var tagSummary = $('<div class="row">' +
                                   '<div class="col-sm-6 text-center">' +
                                   '<a href class="tag-name lead">' + name + '</a>' +
                                   '</div>' +
                                   '<div class="col-sm-6 text-center">' +
                                   '<span class="tag-count lead">' + count + '</span>' +
                                   '</div>' +
                                   '</div>');
